...
export class Painel01Component implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    // Inscreve para receber sinal de rolagem por âncora
    this.anchorSub = this.scrollSrv.anchorScroll$.subscribe(flag => {
      this.anchorScroll = flag;
    });
  ...
  ngOnDestroy(): void {
    ...
    if (this.isLocked) {
      this.unlockScroll(window.scrollY);
    }
  }
  ...
  /* ---------- ESCUTA SCROLL GLOBAL ---------- */
  @HostListener('window:scroll')
  onWindowScroll(): void {
    // 1) Se foi uma rolagem programática por âncora:
    if (this.anchorScroll) {
      // pula direto para o estágio final
      this.zone.run(() => {
        this.stage = 3;
      });
      // limpa a flag para não interferir em rolagens manuais futuras
      this.scrollSrv.clear();
      this.anchorScroll = false;
      return;
    }

    // 2) Scroll manual: mantém o funcionamento normal
    if (this.isLocked) {
      return;
    }
  ...
  private scrub(deltaY: number): void {
    this.progress = Math.max(0, Math.min(1, this.progress + deltaY / this.PIXELS_PER_FULL_PROGRESS));
    this.updateStageFromProgress();
